...
export default function InstitutionsDirectoryClient({
  institutions,
}: {
            ...
            {list.length ? (
              list.map((i) => (
                <Link key={i.id} href={`/institutions/directory/${i.id}`} className="card">
                  <div
                    className="card-logo"
                    style={i.logoUrl ? { backgroundImage: `url(${i.logoUrl})`, backgroundSize: "contain", backgroundRepeat: "no-repeat", backgroundPosition: "center" } : undefined}
                  >
                    {!i.logoUrl && i.logo}
                  </div>
